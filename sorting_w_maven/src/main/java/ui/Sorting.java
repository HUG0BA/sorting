...
public class Sorting<T extends Comparable<T>> {
    ...
    private int Partition(T[] arr, int leftIndex, int rightIndex){
        while (true){
            while(leftIndex < rightIndex && arr[leftIndex].compareTo(arr[rightIndex]) < 0) rightIndex--;
            if(leftIndex < rightIndex) Swap(arr, leftIndex, rightIndex);
            else return leftIndex;

            while(leftIndex < rightIndex && arr[leftIndex].compareTo(arr[rightIndex]) < 0) leftIndex++;
            if(leftIndex < rightIndex) Swap(arr, leftIndex, rightIndex);
